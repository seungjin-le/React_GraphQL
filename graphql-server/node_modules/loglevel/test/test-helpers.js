...
define(function () {
  ...
  self.toBeAtLevel = function toBeAtLevel(level) {
    ...
    var expectedWorkingCalls =
      log.levels.SILENT - log.levels[level.toUpperCase()];
    ...
    expect(realLogMethod.calls.length - priorCalls).toEqual(
      expectedWorkingCalls
    );
    return true;
  };
  ...
  self.toBeTheLevelStoredByCookie = function toBeTheLevelStoredByCookie(name) {
    var level =
      this.actual === undefined ? undefined : this.actual.toUpperCase();
    var storageKey = encodeURIComponent(getStorageKey(name));

    if (level === undefined) {
      return window.document.cookie.indexOf(storageKey + '=') === -1;
    } else if (
      window.document.cookie.indexOf(storageKey + '=' + level) !== -1
    ) {
      return true;
    } else {
      return false;
  ...
  self.toBeTheLevelStoredByLocalStorage =
    function toBeTheLevelStoredByLocalStorage(name) {
      var level =
        this.actual === undefined ? undefined : this.actual.toUpperCase();
  ...
  self.toBeTheStoredLevel = function toBeTheStoredLevel(name) {
    return (
      self.toBeTheLevelStoredByLocalStorage.call(this, name) ||
  ...
  self.setCookieStoredLevel = function setCookieStoredLevel(level, name) {
    window.document.cookie =
      encodeURIComponent(getStorageKey(name)) + '=' + level.toUpperCase() + ';';
  };

  self.setLocalStorageStoredLevel = function setLocalStorageStoredLevel(
    level,
    name
  ) {
  ...
  self.clearStoredLevels = function clearStoredLevels() {
    ...
    if (self.isCookieStorageAvailable()) {
      var storedKeys = window.document.cookie.match(
        /(?:^|;\s)(loglevel(\:\w+)?)(?=\=)/g
      );
      if (storedKeys) {
        for (var i = 0; i < storedKeys.length; i++) {
          window.document.cookie =
            storedKeys[i] + '=; expires=Thu, 01 Jan 1970 00:00:01 GMT;';
        }
      }
    }
  };
