...
function isBadUserInputGraphQLError(error) {
  var _a;
  return (
    ((_a = error.nodes) === null || _a === void 0 ? void 0 : _a.length) === 1 &&
...
async function processGraphQLRequest(config, requestContext) {
  ...
  if (
    ...
    if (!config.persistedQueries || !config.persistedQueries.cache) {
      return await sendErrorResponse(
        new apollo_server_errors_1.PersistedQueryNotSupportedError()
      );
    ...
    if (
      !(
        persistedQueryCache instanceof
        utils_keyvaluecache_1.PrefixingKeyValueCache
      )
    ) {
      persistedQueryCache = new utils_keyvaluecache_1.PrefixingKeyValueCache(
        persistedQueryCache,
        exports.APQ_CACHE_PREFIX
      );
    }
    ...
    if (query === undefined) {
      ...
      if (query) {
        ...
        return await sendErrorResponse(
          new apollo_server_errors_1.PersistedQueryNotFoundError()
        );
      }
    } else {
  ...
  if (!requestContext.document) {
    ...
    try {
      ...
      return await sendErrorResponse(
        syntaxError,
        apollo_server_errors_1.SyntaxError
      );
    }
    ...
    if (validationErrors.length === 0) {
      ...
      return await sendErrorResponse(
        validationErrors,
        apollo_server_errors_1.ValidationError
      );
    }
    if (config.documentStore) {
      Promise.resolve(
  ...
  const operation = (0, graphql_1.getOperationAST)(
  ...
  requestContext.operationName =
    ((_a =
      operation === null || operation === void 0 ? void 0 : operation.name) ===
      null || _a === void 0
      ? void 0
      : _a.value) || null;
  try {
  ...
  let response = await dispatcher.invokeHooksUntilNonNull(
    'responseForOperation',
    requestContext
  );
  if (response == null) {
    const executionListeners = [];
    (await dispatcher.invokeHook('executionDidStart', requestContext)).forEach(
      (executionListener) => {
    ...
    if (executionDispatcher.hasHook('willResolveField')) {
      const invokeWillResolveField = (...args) =>
        executionDispatcher.invokeSyncDidStartHook('willResolveField', ...args);
      Object.defineProperty(
        requestContext.context,
        schemaInstrumentation_1.symbolExecutionDispatcherWillResolveField,
        { value: invokeWillResolveField }
      );
      if (config.fieldResolver) {
        Object.defineProperty(
          requestContext.context,
          schemaInstrumentation_1.symbolUserFieldResolver,
          {
            value: config.fieldResolver,
          }
        );
      }
      (0, schemaInstrumentation_1.enablePluginsForSchemaResolvers)(
        config.schema
      );
    }
    try {
  ...
  async function sendErrorResponse(errorOrErrors, errorClass) {
    ...
    const response = {
      errors: formatErrors(
        errors.map((err) =>
          err instanceof apollo_server_errors_1.ApolloError && !errorClass
            ? err
            : (0, apollo_server_errors_1.fromGraphQLError)(
                err,
                errorClass && {
                  errorClass,
                }
    ...
    if (
      errors.every(
        (err) =>
          err instanceof
            apollo_server_errors_1.PersistedQueryNotSupportedError ||
      ...
      errors.length === 1 &&
      errors[0] instanceof runHttpQuery_1.HttpQueryError
    ) {
      response.http = {
