...
class ApolloServerBase {
  constructor(config) {
    ...
    this.config = {
      ...config,
      nodeEnv:
        (_a = config.nodeEnv) !== null && _a !== void 0
          ? _a
    ...
    this.apolloConfig = (0, determineApolloConfig_1.determineApolloConfig)(
      apollo
    );
    ...
    this.csrfPreventionRequestHeaders =
      csrfPrevention === true
        ...
        ? null
        : (_b = csrfPrevention.requestHeaders) !== null && _b !== void 0
        ? _b
        : recommendedCsrfPreventionRequestHeaders;
    ...
    if (requestOptions.persistedQueries !== false) {
      ...
      requestOptions.persistedQueries = {
        cache: new utils_keyvaluecache_1.PrefixingKeyValueCache(
          apqCache,
          requestPipeline_1.APQ_CACHE_PREFIX
        ),
        ...apqOtherOptions,
      };
    } else {
  ...
  async _start() {
    ...
    try {
      ...
      if (
        (_a = this.requestOptions.persistedQueries) === null || _a === void 0
          ? void 0
          : _a.cache
      ) {
      ...
      const serverWillStops = taggedServerListeners.flatMap((l) =>
        l.serverListener.serverWillStop ? [l.serverListener.serverWillStop] : []
      );
      ...
      if (taggedServerListenersWithRenderLandingPage.length > 1) {
        ...
        this.landingPage =
          await taggedServerListenersWithRenderLandingPage[0].serverListener.renderLandingPage();
      } else {
        this.landingPage = null;
      }
      this.state = {
  ...
  constructSchema() {
    ...
    if (modules) {
      const { schema, errors } = (0, apollo_tools_1.buildServiceDefinition)(
        modules
      );
  ...
  async stop() {
    ...
    try {
      await ((_a = this.drainServers) === null || _a === void 0
        ? void 0
        : _a.call(this));
  ...
  ensurePluginInstantiation(userPlugins = [], isDev) {
    ...
    const alreadyHavePluginWithInternalId = (id) =>
      this.plugins.some(
        (p) =>
          (0, internalPlugin_1.pluginIsInternal)(p) &&
          p.__internal_plugin_id__() === id
      );
    {
      ...
      if (!alreadyHavePlugin && this.apolloConfig.key) {
        if (this.apolloConfig.graphRef) {
          this.plugins.unshift(
            (0, plugin_1.ApolloServerPluginUsageReporting)()
          );
        } else {
          this.logger.warn(
    ...
    }
    {
      ...
      if (!alreadyHavePlugin && enabledViaEnvVar) {
        if (this.apolloConfig.key) {
          const options = {};
          this.plugins.push(
            (0, plugin_1.ApolloServerPluginSchemaReporting)(options)
          );
        } else {
          throw new Error(
    ...
    }
    {
      ...
      if (!alreadyHavePlugin) {
        this.plugins.push(
          (0, plugin_1.ApolloServerPluginInlineTrace)({
