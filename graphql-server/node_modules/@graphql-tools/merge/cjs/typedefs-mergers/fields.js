...
function mergeFields(type, f1, f2, config) {
    ...
    for (const field of f1) {
      if (fieldAlreadyExists(result, field, config)) {
        ...
        if (
          ...
          if (
            (0, utils_js_1.isNonNullTypeNode)(field.type) &&
            !(0, utils_js_1.isNonNullTypeNode)(existing.type)
          ) {
            existing.type = field.type;
          }
        }
        existing.arguments = (0, arguments_js_1.mergeArguments)(
          field['arguments'] || [],
          existing.arguments || [],
          config
        );
        existing.directives = (0, directives_js_1.mergeDirectives)(
          field.directives,
          existing.directives,
          config
        );
  ...
  if (config && config.exclusions) {
    ...
    return result.filter(
      (field) => !exclusions.includes(`${type.name.value}.${field.name.value}`)
    );
  }
  return result;
}
...
function preventConflicts(type, a, b, ignoreNullability = false) {
  ...
  if (
    aType !== bType &&
    !safeChangeForFieldType(a.type, b.type, ignoreNullability)
  ) {
    throw new Error(
...
function safeChangeForFieldType(oldType, newType, ignoreNullability = false) {
  // both are named
  if (
    !(0, utils_js_1.isWrappingTypeNode)(oldType) &&
    !(0, utils_js_1.isWrappingTypeNode)(newType)
  ) {
    return oldType.toString() === newType.toString();
  }
  // new is non-null
  if ((0, utils_js_1.isNonNullTypeNode)(newType)) {
    const ofType = (0, utils_js_1.isNonNullTypeNode)(oldType)
      ? oldType.type
      : oldType;
  ...
  if ((0, utils_js_1.isListTypeNode)(oldType)) {
    return (
      ((0, utils_js_1.isListTypeNode)(newType) &&
        safeChangeForFieldType(oldType.type, newType.type)) ||
      ((0, utils_js_1.isNonNullTypeNode)(newType) &&
