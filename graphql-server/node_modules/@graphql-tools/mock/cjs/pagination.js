...
const relayStylePaginationMock = (
  store,
  { cursorFn = (node) => `${node.$ref.key}`, applyOnNodes, allNodesFn } = {}
) => {
  return (parent, args, context, info) => {
    const source = (0, utils_js_1.isRootType)(info.parentType, info.schema)
      ...
      : parent;
    const allNodesFn_ =
      allNodesFn !== null && allNodesFn !== void 0
        ? allNodesFn
        : defaultAllNodesFn(store);
    ...
    if (typeof first === 'number') {
      ...
      if (last || before) {
        throw new Error(
          "if `first` is provided, `last` or `before` can't be provided"
        );
      }
      const afterIndex = after
        ? allEdges.findIndex((e) => e.cursor === after)
        : -1;
      start = afterIndex + 1;
      ...
      if (first || after) {
        throw new Error(
          "if `last` is provided, `first` or `after` can't be provided"
        );
      }
      const foundBeforeIndex = before
        ? allEdges.findIndex((e) => e.cursor === before)
        : -1;
      const beforeIndex =
        foundBeforeIndex !== -1 ? foundBeforeIndex : allNodes.length;
...
const defaultAllNodesFn = (store) => (parent, _, __, info) =>
  store.get(parent, [info.fieldName, 'edges']).map((e) => store.get(e, 'node'));
