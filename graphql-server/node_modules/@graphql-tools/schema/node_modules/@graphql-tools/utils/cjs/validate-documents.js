...
exports.createDefaultRules =
  exports.checkValidationErrors =
  exports.validateGraphQlDocuments =
    void 0;
...
async function validateGraphQlDocuments(
  ...
  await Promise.all(
    documentFileObjectsToValidate.map(async (documentFile) => {
      ...
      const errors = (0, graphql_1.validate)(
        schema,
        documentToValidate,
        effectiveRules
      );
...
function checkValidationErrors(loadDocumentErrors) {
  if (loadDocumentErrors.length > 0) {
    ...
    throw new AggregateError_js_1.AggregateError(
      errors,
      `GraphQL Document Validation failed with ${errors.length} errors;
  ${errors
    .map((error, index) => `Error ${index}: ${error.stack}`)
    .join('\n\n')}`
    );
  }
}
