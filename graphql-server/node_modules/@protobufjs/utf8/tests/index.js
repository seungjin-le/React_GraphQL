...
tape.test('utf8', function (test) {
  test.test(test.name + ' - length', function (test) {
    test.equal(
      utf8.length(''),
      0,
      'should return a byte length of zero for an empty string'
    );
  ...
  test.test(test.name + ' - write', function (test) {
    ...
    test.equal(
      utf8.write('', buf, 0),
      0,
      'should encode an empty string to an empty buffer'
    );

    var len = utf8.length(dataStr);
    buf = new Buffer(len);
    test.equal(
      utf8.write(dataStr, buf, 0),
      len,
      'should encode to exactly ' + len + ' bytes'
    );
    ...
    for (var i = 0; i < buf.length; ++i) {
      if (buf[i] !== data[i]) {
        test.fail(
          'should encode to the same buffer data as node buffers (offset ' +
            i +
            ')'
        );
        return;
