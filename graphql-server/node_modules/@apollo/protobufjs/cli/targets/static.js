...
static_target.description =
  'Static code without reflection (non-functional on its own)';
...
function exportName(object, asInterface) {
  ...
  return (object[asInterface ? '__interfaceName' : '__exportName'] =
    parts.join('.'));
}
...
function buildFunction(type, functionName, gen, scope) {
    ...
    enter: function (node, parent) {
      ...
      if (
        ...
        return {
          ...
          name:
            '$root' +
            type.fieldsArray[node.property.value].resolvedType.fullName,
        };
      return undefined;
    },
  });
  ...
  if (isCtor)
    ...
    push(
      escapeName(type.name) + '.' + escapeName(functionName) + ' = ' + lines[0]
    );
...
function buildType(ref, type) {
  ...
  type.fieldsArray.forEach(function (field) {
    ...
    if (field.repeated)
      push(
        escapeName(type.name) + '.prototype' + prop + ' = $util.emptyArray;'
      ); // overwritten in constructor
    else if (field.map)
      push(
        escapeName(type.name) + '.prototype' + prop + ' = $util.emptyObject;'
      ); // overwritten in constructor
    else if (field.long)
      push(
        escapeName(type.name) +
  ...
  if (config.decode) {
    ...
    if (config.delimited) {
      ...
      push(
        escapeName(type.name) +
          '.decodeDelimited = function decodeDelimited(reader) {'
      );
      ++indent;
...
function buildEnum(ref, enm) {
  ...
  push(
    (config.es6 ? 'const' : 'var') +
      ' valuesById = {}, values = Object.create(valuesById);'
  );
  var aliased = [];
  Object.keys(enm.values).forEach(function (key) {
