  ...
  if (ranges && ranges.length) {
    ...
    ranges.forEach(function (range) {
      if (typeof range === 'string') parts.push('"' + escape(range) + '"');
        ...
        parts.push(
          range[0] + ' to ' + (range[1] === 0x1fffffff ? 'max' : range[1])
        );
    });
    push('');
...
function buildField(field, passExtend) {
  if (
    ...
    (field.extend !== undefined && !passExtend)
  )
    return;
  if (first) {
    first = false;
...
function buildGroup(field) {
  push(
    field.rule + ' group ' + field.resolvedType.name + ' = ' + field.id + ' {'
  );
  ++indent;
...
function buildFieldOptions(field) {
  ...
  keys.forEach(function (key) {
    ...
    var wireType =
      types.packed[field.resolvedType instanceof Enum ? 'int32' : field.type];
    switch (key) {
      case 'packed':
        val = Boolean(val);
      ...
      case 'default':
        ...
        if (
          (field.long &&
            !util.longNeq(field.defaultValue, types.defaults[field.type])) ||
