...
function modExists(name, version) {
  for (var i = 0; i < module.paths.length; ++i) {
    try {
      var pkg = JSON.parse(
        fs.readFileSync(path.join(module.paths[i], name, 'package.json'))
      );
      return semver
        ? semver.satisfies(pkg.version, version)
        : parseInt(pkg.version, 10) ===
            parseInt(version.replace(/^[\^~]/, ''), 10); // used for semver only
    } catch (e) {
      /**/
    }
  }
...
function modInstall(install) {
  child_process.execSync(
    'npm --silent install ' +
      (typeof install === 'string' ? install : install.join(' ')),
    {
      cwd: __dirname,
      stdio: 'ignore',
    }
...
exports.wrap = function (OUTPUT, options) {
  ...
  try {
    ...
    wrap = fs
      .readFileSync(path.join(__dirname, 'wrappers', name + '.js'))
      .toString('utf8');
  } catch (e) {
  ...
  wrap = wrap.replace(
    /\$DEPENDENCY/g,
    JSON.stringify(options.dependency || '@apollo/protobufjs')
  );
